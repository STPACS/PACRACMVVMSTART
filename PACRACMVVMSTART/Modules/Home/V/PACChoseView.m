...
-(void)selectedItem:(UIButton *)sender{
    ...
    if ([self.delegate respondsToSelector:@selector(pacNavChooseView:clickItem:withIndex:)]) {
        [self.delegate pacNavChooseView:self clickItem:_modelAry[sender.tag - 10] withIndex:sender.tag-10];
    }
